...
int FxRedisClient::Query(const char* pszCMD)
{
	if (false == m_bDbOK)
	{
		return -1;
...
int FxRedisClient::Query(const char* pszCMD, IRedisDataReader **ppReader)
{
    if (NULL != *ppReader)
    {
        return -1;
    ...
    *ppReader = poReader;
	...
	int nRetCode = m_poMySqlConn->Query( pszCMD, *poReader);
