...
SOCKET FxTCPListenSock::Listen(unsigned int dwIP, unsigned short& wPort)
{
	...
	for (int i = 0; i < sizeof(m_oSPerIoDatas) / sizeof(m_oSPerIoDatas[0]); i++)
	{
...
bool FxTCPListenSock::PushNetEvent(ENetEvtType eType, unsigned int dwValue)
{
...
void FxTCPListenSock::__ProcError(unsigned int dwErrorNo)
{
...
bool FxTCPListenSock::PostAccept(SPerIoData& oSPerIoData)
{
	...
	{
		int dwErr = WSAGetLastError();
		ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "Set socket FIONBIO error : %d", dwErr);
	...
	if (false == bRet)
	{
		...
		{
			ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "Init AcceptEx failed, errno %d", nError);
...
void FxTCPListenSock::OnParserIoEvent(bool bRet, void* pIoData, unsigned int dwByteTransferred)
{
...
void FxTCPListenSock::OnAccept()
{
	...
	unsigned int hAcceptSock = accept(GetSock(), (sockaddr*)&stRemoteAddr, &dwAddrLen);
	if (INVALID_SOCKET == hAcceptSock)
	{
		ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "%s", "INVALID_SOCKET == hAcceptSock");
	...
	if (NULL == poSock)
	{
		ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "%s", "create FxConnectSock failed");
	...
	if (NULL == poConnection)
	{
		ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "%s", "NULL == poConnection");
	...
	if (NULL == poSession)
	{
		ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "%s", "NULL == poSession");
	...
	{
		ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "%s", "NULL == poEpollHandler");
	...
	int nFlags = fcntl(hAcceptSock, F_GETFL, 0);
	nFlags |= O_NONBLOCK;
	fcntl(hAcceptSock, F_SETFL, nFlags);

	if (!poSock->AddEvent())
	{
		ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "%s", "poSock->AddEvent() failed");
...
void FxWebSocketListen::OnAccept(SPerIoData* pstPerIoData)
{
...

	{
		...
		if (NULL == poSession)
		{
			ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "CreateSession failed");
		...
		if (NULL == poIoThreadHandler)
		{
			ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "get iothread failed");
...
bool FxTCPConnectSockBase::PushNetEvent(ENetEvtType eType, unsigned int dwValue)
{
	SNetEvent oEvent;
	// 先扔网络事件进去，然后在报告上层有事件，先后顺序不能错，这样上层就不会错取事件//
	oEvent.eType = eType;
...
bool FxTCPConnectSockBase::PostSend()
{
	...
	if (0 >= nLen)
	{
		//ThreadLog(LogLv_Critical, m_poIoThreadHandler->GetFile(), "m_poSendBuf->GetOutCursorPtr() = %d, socket : %d, socket id : %d", nLen, GetSock(), GetSockId());
	...
	{
		//		ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "m_poSendBuf->GetOutCursorPtr() = %d, socket : %d, socket id : %d", nLen, GetSock(), GetSockId());
		// 不算失败，只是没有投递而已，下次可以继续//
...
bool FxTCPConnectSockBase::PostSendThread()
{
	if (false == IsConnected())
	{
		ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "false == IsConnect(), socket : %d, socket id : %d", GetSock(), GetSockId());
	...
	if (0 != nRet)
	{
		if (WSAGetLastError() != WSA_IO_PENDING)
		{
			...
			unsigned int dwErr = WSAGetLastError();
			ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "WSASend errno : %d, socket : %d, socket id : %d", WSAGetLastError(), GetSock(), GetSockId());
...
void FxTCPConnectSockBase::__ProcConnectError(unsigned int dwErrorNo)
{
	if (GetConnection())
	{
		if (GetSockId() != GetConnection()->GetID())
...
void FxTCPConnectSockBase::__ProcError(unsigned int dwErrorNo)
{
	if (GetConnection())
	{
		if (GetSockId() != GetConnection()->GetID())
...
void FxTCPConnectSockBase::OnConnect()
{
	...
	{
		ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "getsockopt errno : %d, socket : %d, socket id : %d", errno, GetSock(), GetSockId());
	...
	if (nError != 0)
	{
		ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "getsockopt errno : %d, socket : %d, socket id : %d", nError, GetSock(), GetSockId());
	...
	sockaddr_in stAddr = { 0 };
	unsigned int dwAddrLen = sizeof(stAddr);
	...
	{
		ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "m_poIoThreadHandler->ChangeEvent, socket : %d, socket id : %d", GetSock(), GetSockId());
...
void FxTCPConnectSockBase::OnRecv(bool bRet, int dwBytes)
{
	...
	if (false == bRet)
	{
		ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "false == bRet errno : %d, socket : %d, socket id : %d", WSAGetLastError(), GetSock(), GetSockId());
	...
	{
		//ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "m_poRecvBuf->CostBuff error");
		//
		//InterlockedCompareExchange(&m_nPostRecv, 0, m_nPostRecv);
	...
	if (nLen <= 0)
	{
		ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "m_poRecvBuf->GetUsedCursorPtr <= 0, socket : %d, socket id : %d", GetSock(), GetSockId());
...
bool FxTCPConnectSockBase::PostRecv()
{
	...
	if (0 >= nLen)
	{
		//ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "m_poRecvBuf->GetInCursorPtr() = %d, socket : %d, socket id : %d", nLen, GetSock(), GetSockId());
	...
	if (0 != WSARecv(GetSock(), &m_stRecvIoData.stWsaBuf, 1, &dwReadLen, &dwFlags, &m_stRecvIoData.stOverlapped, NULL))
	{
		if (WSAGetLastError() != WSA_IO_PENDING)
		{
			ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "WSARecv errno : %d, socket : %d, socket id : %d", WSAGetLastError(), GetSock(), GetSockId());
...
bool FxTCPConnectSockBase::PostRecvFree()
{
	...
	if (0 != nPostRecv)
	{
		ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "m_nPostRecv == 1, socket : %d, socket id : %d", GetSock(), GetSockId());
		return false;
	}
	ZeroMemory(&m_stRecvIoData.stOverlapped, sizeof(m_stRecvIoData.stOverlapped));
	//post 失败的时候 再进入这个函数 可能会丢失一次
	if (!PostQueuedCompletionStatus(m_poIoThreadHandler->GetHandle(), unsigned int(-1), (ULONG_PTR)this, &m_stRecvIoData.stOverlapped))
	{
...
SOCKET FxTCPConnectSock::Connect()
{
	...
	if (-1 == connect(GetSock(), (sockaddr*)&stAddr, sizeof(stAddr)))
	{
		if (errno != EINPROGRESS && errno != EINTR && errno != EAGAIN)
		{
			...
			ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "connect error id : %d, socket : %d, socket id : %d", errno, GetSock(), GetSockId());
			Close();
			return INVALID_SOCKET;
		}
	}
...
void FxTCPConnectSock::__ProcRecv(unsigned int dwLen)
{
	if (GetConnection())
	{
...
void FxTCPConnectSock::__ProcRelease()
{
	if (GetConnection())
	{
		if (GetSockId() != GetConnection()->GetID())
		{
			ThreadLog(LogLv_Error, m_poIoThreadHandler->GetFile(), "socket : %d, socket id : %d, connection id : %d, connection addr : %p", GetSock(), GetSockId(), GetConnection()->GetID(), GetConnection());
			return;
		}
...
void FxWebSocketConnect::__ProcRecv(unsigned int dwLen)
{
	if (GetConnection())
	{
		if ((unsigned int)(-1) == dwLen)
...
void FxHttpConnect::__ProcRecv(unsigned int dwLen)
{
	if (GetConnection())
	{
